...
class VentaDAOTest {
    ...
    void executeSale(int i) {
        ...
        VentaDTO venta = new VentaDTO(productos[i], cantidades[i], ruts[i], fechas[i], totales[i]);
        new VentaDAO(venta).executeSale(saleMaxStock[i]);
        VentaDTO ventaDB = new VentaDAO().getVentaById(id);

        if(i < 2){
            venta = new VentaDTO(productos[i], cantidadVendida[i], ruts[i], ventaDB.getFecha(), totales[i]);
    ...
    void editSaleById(int i) {
        ...
        VentaDTO ventaDB = new VentaDAO().getVentaById(id);
        VentaDTO venta = new VentaDTO(productos[i], cantidades[i], ruts[i], ventaDB.getFecha() , totales[i]);
    ...
    void getVentasDB() {
        ...
        for(int i = 0; i < ventasDB.size(); i++){
            assertEquals(ventasDB.get(i), new VentaDTO(productos[i], cantidades[i], ruts[i], ventasDB.get(i).getFecha(), totales[i]));
        }
    }
